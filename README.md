...
4. **Open Depot Protocol**: signed catalog syndication (`GET /odp/catalog`, `POST /odp/subscribe`) with signature verification before merge.

## Status

Implemented: the routes above, signed and tamper-checked descriptors, friend persistence, presence/peer tracking, optional encrypted-at-rest key storage, and ODP catalog merge with signature verification.

Not implemented yet: peer transport / NAT traversal ([#4](https://github.com/Heretek-Games/drop-federation/issues/4)), direct or group messaging ([#5](https://github.com/Heretek-Games/drop-federation/issues/5)), opt-in library/save sharing ([#6](https://github.com/Heretek-Games/drop-federation/issues/6)), key rotation, and the full threat model ([#7](https://github.com/Heretek-Games/drop-federation/issues/7)). There is no peer-to-peer messaging today.
